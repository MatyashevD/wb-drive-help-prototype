...
// Демонстрирует 4 состояния прогресса и bottom sheet с 3-мя
// ветками: ЧП, плановое досрочное, случайно тапнул.
// ============================================================
...
(function () {
    ...
    const state = {
        ...
        // Вариант оформления досрочного завершения (для A/B-сравнения):
        // 'link'    — мелкая текстовая ссылка над Навигатором (default)
        // 'iconBtn' — красная иконка-кнопка «⚠» слева от Навигатора
        earlyFinishVariant: 'link',
        sheet: null,         // null | 'causes' | 'emergency' | 'planned' | 'returns-warn'
        success: null,       // null | 'completed' | 'planned-early' | 'emergency' | 'completed-with-returns'
        allPointsOpen: false,
    };
    ...
    function renderBottomActions() {
        ...
        // ─── Кейс A: всё разгружено + возвратов нет → primary «Завершить поездку» ───
        ...
        // ─── Кейс B: всё разгружено + есть возвраты + на складе → primary «Сдать возвраты и завершить» ───
        ...
        // ─── Кейс C: всё разгружено + есть возвраты + НЕ на складе → синяя информационная «Везите возвраты на склад» ───
        if (allDelivered && hasReturns && !atWarehouse) {
            return `
        ...
        // ─── Кейс D: поездка ещё идёт → доступ к «Завершить досрочно» зависит от варианта ───
        const v = state.earlyFinishVariant;

        // Вариант B: красная иконка-кнопка «⚠» слева от Навигатора
        if (v === 'iconBtn') {
            return `
                <div class="fr-bottom-row">
                    <button class="fr-finish-icon" id="finishBtn" aria-label="Завершить досрочно">
                        ${svg('i-warning', 22)}
                    </button>
                    <button class="fr-navigator" aria-label="Навигатор">
                        ${svg('i-navigator', 22)}<span>Навигатор</span>
        ...
        // Вариант A (default): текстовая ссылка над большим Навигатором
    ...
    // ============================================================
    // Bottom sheet: предупреждение «у вас возвраты, вы не на складе»
    // ============================================================
    ...
    // ============================================================
    // Bottom sheet: «Что произошло?»
    // ============================================================
    function renderSheetCauses() {
        const complete = isComplete();
        if (complete) {
            // — Когда всё разгружено, sheet не нужен — это не должно вызываться,
            //   но на случай возврата к этому стейту покажем upbeat-вариант. —
            return `
                <div class="fr-sheet__header">
    ...
    function renderSlideToConfirm(label) {
        return `
            <div class="fr-slide" id="slide" data-label="${label}">
                <div class="fr-slide__fill" id="slideFill"></div>
                <div class="fr-slide__track-text" id="slideText">${label}</div>
    ...
    function renderSuccess(kind) {
        ...
        const totalAtDriverEarly = unloaded + returnsCount;
        ...
        const map = {
            ...
            'planned-early': {
                ...
                sub: hasReturns
                    ? `${totalAtDriverEarly} ${pluralize(totalAtDriverEarly,'коробка остаётся','коробки остаются','коробок остаются')} за вами (${unloaded} невыгруженных + ${RETURNS_COUNT} возвратных). Довезите их на склад до конца смены — логист увидит причину в отчёте.`
    ...
    // ============================================================
    function wireEvents() {
        // Кнопка «Завершить» внизу — поведение зависит от кейса
        ...
        if (finishBtn) {
            finishBtn.addEventListener('click', () => {
                ...
                // Кейс D: маршрут не закончен → bottom sheet с причинами
                openSheet('causes');
            });
        }
    ...
    function wireSheetEvents() {
        // Закрытие (×, бэкдроп, кнопка «Вернуться»)
        const closes = ['sheetClose', 'sheetClose2'];
        closes.forEach(id => {
        ...
        // Кнопка уведомления логиста (ЧП-ветка)
        ...
        // Кнопка «Позвонить в 112» (открывает дозвонщик — в прототипе просто пометка)
        const call112Btn = document.getElementById('call112');
        if (call112Btn) {
            call112Btn.addEventListener('click', () => {
                // В реальном приложении: window.location.href = 'tel:112'
        ...
        // Кнопка «Построить маршрут до склада» (в returns-warn sheet)
        ...
        // Confirm completed (на случай если sheet вызвали в финальном стейте)
        ...
        if (state.sheet === 'emergency') {
            wireSlide(() => {
                closeSheet();
                renderSuccess('emergency');
            });
        }
        if (state.sheet === 'planned') {
            wireSlide(() => {
                closeSheet();
                renderSuccess('planned-early');
            });
        }
    }
    ...
    function renderStateSwitcher() {
        ...
        container.innerHTML = `
            ...
            ${VARIANTS.map(v => `
                <button class="state-switcher__btn ${state.earlyFinishVariant === v.id ? 'is-active' : ''}" data-action="set-variant" data-variant="${v.id}">${v.label}</button>
            `).join('')}
        `;
